...
def get_clean_data(json_path: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    Extracts and cleans data from a JSON file.

    Args:
        json_path (str): Path to the JSON file.

    Returns:
        Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]: A tuple containing the cleaned data and all nodes.
    """
    ...
    def get_all_nodes(data: List[Dict[str, Any]]) -> None:
        for node in data:
            extracted = {
...
def judge(bbox1: List[int], bbox2: List[int], size: Tuple[int, int]) -> bool:
    """
    Determines if bounding boxes are invalid based on the image size.

    Args:
        bbox1 (List[int]): First bounding box [left, top, right, bottom].
        bbox2 (List[int]): Second bounding box [left, top, right, bottom].
        size (Tuple[int, int]): Image size (width, height).

    Returns:
        bool: True if any coordinate is out of bounds, False otherwise.
    """
...
def get_resolution(image_path: str) -> Tuple[int, int]:
    """
...
def unify_bbox(bbox: List[int], size: Tuple[int, int]) -> List[float]:
    """
    Normalizes bounding box coordinates based on image size.
    ...
    Returns:
        List[float]: Normalized bounding box coordinates.
    """
    w, h = size
    return [bbox[0]/w, bbox[1]/h, bbox[2]/w, bbox[3]/h]

def extract_components(
    data: List[Dict[str, Any]], parent: Optional[Dict[str, Any]] = None
) -> List[Dict[str, Any]]:
    """
    Recursively extracts component information from data.

    Args:
        data (List[Dict[str, Any]]): List of component data.
        parent (Optional[Dict[str, Any]]): Parent component.

    Returns:
        List[Dict[str, Any]]: Extracted components.
    """
    components = []
...
def get_negatives(
    ...
    """
    Generates negative samples based on the specified relation type.

    Args:
        node (Dict[str, Any]): Current node.
        parent (Dict[str, Any]): Parent node.
        size (Tuple[int, int]): Image size (width, height).
        relation_type (str, optional): Type of relation. Defaults to '4sibling'.
        all_nodes (Optional[List[Dict[str, Any]]], optional): All nodes. Defaults to None.
        siblings (Optional[List[Dict[str, Any]]], optional): Sibling nodes of the current node(icon/widget/text/image etc). Defaults to None.
        children (Optional[List[Dict[str, Any]]], optional): Child nodes. Defaults to None.

    Returns:
        Optional[Tuple[Dict[str, Any], int, Dict[str, Any]]]: Negative sample tuple or None.
    """
    w, h = size[0], size[1]
...
def parse_json_hierarchy(
    ...
    """
    Parses the JSON hierarchy to extract relational data.

    Args:
        data (List[Dict[str, Any]]): Cleaned data.
        size (Tuple[int, int]): Image size (width, height).
        num (int): Number of samples to generate.
        all_nodes (List[Dict[str, Any]]): All nodes in the data.

    Returns:
        List[Any]: List of relational tuples.
    """

    result = []

    def dfs(node: Dict[str, Any], parent: Optional[Dict[str, Any]] = None) -> None:
...
def unif_bbox(bbox: List[int], source_size: Tuple[int, int], scale: int) -> List[int]:
    """
    Scales and normalizes bounding box coordinates.

    Args:
        bbox (List[int]): Bounding box [left, top, right, bottom].
        source_size (Tuple[int, int]): Source image size (width, height).
    ...
    return [
        int((bbox[0] / width) * scale),
        int((bbox[1] / height) * scale),
        int((bbox[2] / width) * scale),
        int((bbox[3] / height) * scale)
    ]

def get_relation_prediction_data(data_path: str, save_path: str) -> None:
    """
    Generates relation prediction data from JSON annotations.

    Args:
        data_path (str): Path to the training index JSON file.
        save_path (str): Path to save the generated relation data.
    """
    train_pairs = []
    Uni_Labels_List = []
    part = 1
    a,b,c,d = 0,0,0,0
    pos, neg = 7000, 3000 # positive/negative samples(nodes)
