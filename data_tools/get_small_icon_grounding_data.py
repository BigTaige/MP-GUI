...
def process_bbox(bbox, w, h):
    #y,x,h,w
    new_bbox = [ int(bbox[1]*w), int(bbox[0]*h), int((bbox[1]+bbox[3])*w), int((bbox[0]+bbox[2])*h)]
...
def cal_percent(bbox, image_size):
    return (bbox / image_size) * 100 

def for_minicpm(box,w,h):
    return [box[0]*1000//w, box[1]*1000//h, box[2]*1000//w, box[3]*1000//h]

def get_folds(fold='', save_path='', max_rate=0.3):
    types = []
    part = 0
    train_items = []
                ...
                for row in data:
                    ...
                    for x in map_:
                        if len(map_[x]) == 1 and (x not in ['TEXT']):
                            ...
                            sample['rate'] = rate
                            sample['gt'] = for_minicpm(sample['gt'], w, h)
