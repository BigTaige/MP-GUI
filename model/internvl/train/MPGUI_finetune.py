...
from internvl.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      IMG_CONTEXT_TOKEN, IMG_END_TOKEN, EXTRA_IMG_CONTEXT_TOKEN, 
...
class ModelArguments:
    ...
    use_mpgui: Optional[bool] = field(
        default=True,
        metadata={'help': ''}
    )
    train_graphical_perceiver : Optional[bool] = field(
        default=False,
        metadata={'help': ''}
    )
    train_textual_perceiver : Optional[bool] = field(
        default=False,
        metadata={'help': ''}
    )
    train_spatial_perceiver: Optional[bool] = field(
        default=False,
        metadata={'help': ''}
    )
    train_fusion_gate: Optional[bool] = field(
        default=False,
        metadata={'help': ''}
    )
    first_train_graphical: Optional[bool] = field(
        default=False,
        metadata={'help': ''}
    )
    first_train_textual: Optional[bool] = field(
        default=False,
        metadata={'help': ''}
    )
    first_train_spatial: Optional[bool] = field(
        default=False,
        metadata={'help': ''}
    )
    first_train_gate:  Optional[bool] = field(
        default=False,
        metadata={'help': ''}
    )
    use_fusion_gate:  Optional[bool] = field(
        default=False,
        metadata={'help': ''}
    )
...
def main():
    ...
    ### add EXTRA_IMG_CONTEXT_TOKEN
    token_list = [IMG_START_TOKEN, IMG_END_TOKEN, IMG_CONTEXT_TOKEN, EXTRA_IMG_CONTEXT_TOKEN, 
    ...
    if model_args.model_name_or_path is not None:
        ...
        config.train_graphical_perceiver = model_args.train_graphical_perceiver
        config.train_textual_perceiver = model_args.train_textual_perceiver
        config.train_spatial_perceiver = model_args.train_spatial_perceiver
        config.train_fusion_gate = model_args.train_fusion_gate
        config.use_fusion_gate = model_args.use_fusion_gate
        ...
        logger.info('Building MP-GUI...')
        model = MPGUI(internvl_chat_config, vision_model, llm)
    model.img_context_token_id = img_context_token_id

    model.extra_img_context_token_id = tokenizer.convert_tokens_to_ids(EXTRA_IMG_CONTEXT_TOKEN)
    ...
    if model_args.use_mpgui:
        if not model_args.train_graphical_perceiver:
            _freeze_params(model.graphical_perceiver)
        elif model_args.first_train_graphical:
            print('Initial graphical_perceiver ......') 
            model.graphical_perceiver.apply(model.initialize_weights)
            print('finish!!!')
        elif not model_args.first_train_textual:
            print('Loading graphical_perceiver ......')

        if not model_args.train_textual_perceiver:
            _freeze_params(model.textual_perceiver)
        elif model_args.first_train_textual:
            print('Initial textual_perceiver ......') 
            model.textual_perceiver.apply(model.initialize_weights)
            print('finish!!!')
        elif not model_args.first_train_textual:
            print('Loading textual_perceiver ......')

        if not model_args.train_spatial_perceiver:
            _freeze_params(model.spatial_perceiver)
        elif model_args.first_train_spatial:
            print('Initial spatial_perceiver ......') 
            model.spatial_perceiver.apply(model.initialize_weights)
            print('finish!!!')
        elif not model_args.first_train_spatial:
            print('Loading spatial_perceiver ......')
            
        if not model_args.train_fusion_gate:
            _freeze_params(model.fusion_gate)
        elif model_args.first_train_gate:
            model.FusionGate.apply(model.initialize_weights)
            print('finish!!!')
        else:
            print('Loading gate_net from ours......')
