...
def chat(img_url: str = '', query: str = '') -> dict:
    """
    Constructs the payload for a chat request.

    Args:
        img_url (str): URL of the image. Defaults to an empty string.
        query (str): The user query or prompt.

    Returns:
        dict: The constructed data payload for the chat request.
    """
    if img_url != '':
        data = {
...
def resize(img_path: str, task: str) -> None:
    """
    Resizes an image by 70% to avoid OOM problems and saves it with a temporary filename.

    Args:
        img_path (str): Path to the input image.
        task (str): Task identifier used in the temporary filename.
    """
...
def _return_spf_qa(res_json: list, raw_path: str, idx: int = 0) -> None:
    """
    Generates QA pairs based on the icons, text, relationships, and locations in an image.

    Args:
        res_json (list): List to store the resulting QA pairs.
        raw_path (str): Path to the raw image.
        idx (int, optional): Index of the current image. Defaults to 0.
    """
...
def get_spf_qa(img_list: list) -> None:
    """
    Processes a list of images to generate QA pairs and saves the results to a JSON file.
...
def _return_screen(res_json: list, raw_path: str, idx: int = 0) -> None:
    """
    Generates a summary of the screen and appends it to the result JSON list.

    Args:
        res_json (list): List to store the screen summaries.
        raw_path (str): Path to the raw image.
        idx (int, optional): Index of the current image. Defaults to 0.
    """
...
def get_global_des(img_list: list) -> None:
    """
    Generates global descriptions for a list of images and saves the results to a JSON file.
...
def get_yolo_res(model: YOLO, raw_path: str, output_path: str) -> list:
    """
    Retrieves bounding boxes from an image using a YOLO model.

    Args:
        model (YOLO): The YOLO model used for prediction.
        raw_path (str): Path to the input image.
        output_path (str): Path to save any output if needed.

    Returns:
        list: List of bounding boxes normalized by image dimensions.
    """
...
def _return_local_des(res_json: list, bboxes: list, raw_path: str, idx: int = 0) -> None:
    """
    Generates local descriptions for each bounding box in an image and appends them to the result JSON list.

    Args:
        res_json (list): List to store the local descriptions.
